...
const Filter=()=>{
    ...
    const rating=[{id:"4_STARS_AND_ABOVE",ratinglabel:"4 stars and above"},
    ...
    return(
        <div className="flex-column">
            <button className="btn-link padding-sm" onClick={()=>productsDispatch({type:"CLEAR_ALL"})}>Clear All Filters</button>  
            <hr/> 
            ...
            onChange={(e)=>productsDispatch({type:"FILTER_BY_PRICE",payload:e.target.value})}/>
            <p className="padding-sm">Price under: {products.pricerange}</p>

            <h3 className="padding-sm">Brands</h3>
            <div className="padding-sm">
                {brandings.map(element=>(<div key={element.id}>
                    <input type="checkbox" 
                    id={element.id} 
                    name={element.brandname} 
                    checked={products.listofbrands.includes(element.brandname)} 
                    onClick={()=>productsDispatch({type:"FILTER_BY_BRANDS",payload:element.brandname})}/>
            ...
            <div className="padding-sm">
                {rating.map(element=>(<div key={element.id}>
                    <input type="radio" 
                    id={element.id} 
                    name="rating" 
                    value={element.id} 
            ...
            <div className="padding-sm">
                {sortingCategory.map(element=>(<div key={element.id}>
                    <input type="radio" id={element.id} name="sorting" value={element.id} onClick={()=>productsDispatch({type:element.id,payload:element.id})} checked={products.sorting===element.id}/>
